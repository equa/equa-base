#!/bin/bash
# Toolchain smoke test for the equa-base-ci image (T1 — slim checks tier).
# Asserts every checks-tier tool resolves AND that az and the heavy toolchain
# (oneAPI / SBCL / Go) are absent — T1 must stay minimal.
# Run inside the built image:
#   docker run --rm equa-base-ci:test bash /tmp/run-all-ci.sh
...
# Asserts a command is NOT on PATH (az and the heavy toolchain stay in higher tiers).
absent_check() {
    local name="$1"
    local cmd="$2"
    if command -v "$cmd" > /dev/null 2>&1; then
        fail "$name" "$cmd unexpectedly present in T1"
    else
        pass "$name"
    fi
}
...
echo ""
echo "=== az and heavy toolchain must be absent ==="
absent_check "az absent"   az
